...
{
   constructor(host, hostMap, hostUpdate)
   {
      ...
      Object.defineProperties(publicAPI, {
         ...
         size: { get: () => Array.isArray(indexAdapter.index) ? indexAdapter.index.length : 0 }
      });
...
// TODO: Need new hash!
   ...
   initAdapters(filtersAdapter, sortAdapter)
   {
      ...
      this.sortFn = (a, b) =>
      {
         return this.sortAdapter.compareFn(this.hostMap.get(a), this.hostMap.get(b));
      };
   }
   ...
   reduceImpl()
   {
      ...
      for (const key of this.hostMap.keys())
      {
         include = true;

         const value = this.hostMap.get(key);
   ...
   update(force = false)
   {
      ...
      if ((this.filtersAdapter.filters.length === 0 && !this.sortAdapter.compareFn) ||
       (this.indexAdapter.index && this.hostMap.size !== this.indexAdapter.index.length))
      {
      ...
      if (this.sortAdapter.compareFn)
      {
         ...
         if (!this.indexAdapter.index) { this.indexAdapter.index = [...this.hostMap.keys()]; }
